...
int main(int argc, char **argv)
{
    ...
    if((window = SDL_CreateWindow("Image Loading", 100, 100, width/*WIDTH*/, height/*HEIGHT*/, SDL_WINDOW_RESIZABLE | SDL_RENDERER_PRESENTVSYNC)) == NULL)
    {
        return 20;
    }
    //SDL Renderer
    ...
    figuras.push_back("elipce");
    figuras.push_back("cubo");
    figuras.push_back("star");
...
//    texturas["elipce"] = IMG_LoadTexture(renderer,"elipce.png");
//    texturas["star"] = IMG_LoadTexture(renderer,"star.png");
//    texturas["cubo"] = IMG_LoadTexture(renderer,"cubo.png");

    map<string,SDL_Rect*> rectangulos;
//    rectangulos["elipce"] = new SDL_Rect();
//    rectangulos["elipce"]->x=0;
//    rectangulos["elipce"]->y=0;
//    SDL_QueryTexture(texturas["elipce"],NULL,NULL,&rectangulos["elipce"]->w,&rectangulos["elipce"]->h);
//
//    rectangulos["star"] = new SDL_Rect();
//    rectangulos["star"]->x=0;
//    rectangulos["star"]->y=0;
//    SDL_QueryTexture(texturas["star"],NULL,NULL,&rectangulos["star"]->w,&rectangulos["star"]->h);
//
//    rectangulos["cubo"] = new SDL_Rect();
//    rectangulos["cubo"]->x=0;
//    rectangulos["cubo"]->y=0;
//    SDL_QueryTexture(texturas["cubo"],NULL,NULL,&rectangulos["cubo"]->w,&rectangulos["cubo"]->h);

    for(int i=0; i<figuras.size(); i++)
    {
        texturas[figuras[i]] = IMG_LoadTexture(renderer,(figuras[i]+".png").c_str());
        rectangulos[figuras[i]] = new SDL_Rect();
        rectangulos[figuras[i]]->x=0;
        rectangulos[figuras[i]]->y=0;
        SDL_QueryTexture(texturas[figuras[i]],NULL,NULL,&(rectangulos[figuras[i]]->w),&(rectangulos[figuras[i]]->h));
    }

    vector<SDL_Texture*> texts;
    vector<SDL_Rect*> rects;

//    texts.push_back(texturas["cubo"]);
//    SDL_Rect *n = new SDL_Rect();
//    SDL_Rect *temp = rectangulos["cubo"];
//    n->h = temp->h;
//    n->w = temp->w;
//    n->x = 0;//rand() % width + 1;
//    n->y = 0;//rand() % height + 1;
//    rects.push_back(rectangulos["cubo"]);
    ...
    while (shutdownClient == false)
    {
        ...
        if (socketActive != 0)
        {
            ...
            if (messageFromServer != 0)
            {
                ...
                for(int i=0; i<figuras.size(); i++)
                {
                    if(strcmp(buffer,figuras[i].c_str())==0)
                    {
                        texts.push_back(texturas[figuras[i]]);
                        SDL_Rect *n = new SDL_Rect();
                        SDL_Rect *temp = rectangulos[figuras[i]];
                        n->h = temp->h;
                        n->w = temp->w;
                        n->x = rand() % width - n->w*0.5;
                        n->y = rand() % height - n->h*0.5;
                        rects.push_back(n);
                    }
                }
...
//        SDL_RenderCopy(renderer, texts[0], NULL, rects[0]);
